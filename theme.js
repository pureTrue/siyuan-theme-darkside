...
/* 颜色配置文件列表 */
window.themecolor.colors = [
    'config/theme-primary-color-1.css',
    'config/theme-primary-color-2.css',
    'config/theme-primary-color-3.css',
	'config/theme-primary-color-4.css',
	'config/theme-primary-color-5.css',
	'config/theme-primary-color-6.css',
	'config/theme-primary-color-7.css',
	'config/theme-primary-color-8.css',
	'config/theme-primary-color-9.css',
];

/* DOM 节点 ID */
window.themecolor.IDs = {
...
/* 循环迭代器 */
window.themecolor.Iterator = function* (items) {
 ...
 */
window.themecolor.loadStyle = function (href, id = null) {
 ...
 */
window.themecolor.updateStyle = function (id, href) {
    let style = document.getElementById(id);
    if (style) {
        style.setAttribute('href', href);
    }
    else {
        window.themecolor.loadStyle(href, id);
    }
}
...
setTimeout(() => {
    ...
    if (window.themecolor.clientMode == "mobile") drag = document.getElementById('toolbarName'); // 手机端的标题栏不太一样
    ...
    if (themeStyle) {
        ...
        const colors_href = [];
        window.themecolor.colors.forEach(color => colors_href.push(`${THEME_ROOT}${color}`));
        window.themecolor.iter = window.themecolor.Iterator(colors_href);
        var color_href = window.siyuan?.storage[window.themecolor.IDs.LOCAL_STORAGE_COLOR_HREF];
        if (!color_href) {
            localStorage.getItem(window.themecolor.IDs.LOCAL_STORAGE_COLOR_HREF);
        }
        if (color_href) { // 将迭代器调整为当前配色
            for (let i = 0; i < window.themecolor.colors.length; ++i) {
                if (window.themecolor.iter.next().value === color_href) break;
            }
        }
        else { // 迭代器第一个为当前配色
            color_href = window.themecolor.iter.next().value;
            localStorage.setItem(window.themecolor.IDs.LOCAL_STORAGE_COLOR_HREF, color_href);
            setLocalStorageVal(window.themecolor.IDs.LOCAL_STORAGE_COLOR_HREF, color_href);
        }

        /* 加载配色文件 */
        window.themecolor.updateStyle(window.themecolor.IDs.STYLE_COLOR, color_href);

        if (window.themecolor.clientMode == "mobile") {
            // 如果是手机端，就直接给标题栏塞个图标，不然样式不对
            const doc = new DOMParser().parseFromString(`<svg id="${window.themecolor.IDs.BUTTON_TOOLBAR_CHANGE_COLOR}" class="toolbar__icon"><use xlink:href="#iconTheme"></use></svg>`, 'text/html')
            const svg_change_color = doc.getElementById(window.themecolor.IDs.BUTTON_TOOLBAR_CHANGE_COLOR)
        ...
        button_change_color.id = window.themecolor.IDs.BUTTON_TOOLBAR_CHANGE_COLOR;
...
async function setLocalStorageVal(ikey, ival) {
    ...
    function themeButton() {
        ...
        // 浅色主题配色
        const lightThemes = [
            { id: "buttonSavor-light", label: "Light-1", path: "/appearance/themes/siyuan-theme-darkside/theme/light-1.css"},
            { id: "buttonsalt", label: "Light-2", path: "/appearance/themes/siyuan-theme-darkside/theme/light-2.css" },
            { id: "buttonsugar", label: "Light-3", path: "/appearance/themes/siyuan-theme-darkside/theme/light-3.css" },
            { id: "buttonforest", label: "Light-4", path: "/appearance/themes/siyuan-theme-darkside/theme/light-4.css" }
        ];
        
        // 深色主题配色
        const darkThemes = [
            { id: "buttonSavor-dark", label: "Dark-1", path: "/appearance/themes/siyuan-theme-darkside/theme/dark-1.css" },
            { id: "buttonvinegar", label: "Dark-2", path: "/appearance/themes/siyuan-theme-darkside/theme/dark-2.css" },
            { id: "buttonocean", label: "Dark-3", path: "/appearance/themes/siyuan-theme-darkside/theme/dark-3.css" },
            { id: "buttonmountain", label: "Dark-4", path: "/appearance/themes/siyuan-theme-darkside/theme/dark-4.css" }
        ];
            ...
            const funs = () => {
                setTimeout(() => {
                    if (isPhone()) {
                        ...
                        if (themeMode == 'light') {
                            const lightThemes = [
                                { id: 'buttonsalt', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/light-2.css', styleId: 'Sv-theme-color-salt' },
                                { id: 'buttonsugar', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/light-3.css', styleId: 'Sv-theme-color-sugar' },
                                { id: 'buttonforest', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/light-4.css', styleId: 'Sv-theme-color-forest' }
                            ];
                        ...
                        if (themeMode == 'dark') {
                            const darkThemes = [
                                { id: 'buttonvinegar', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/dark-2.css', styleId: 'Sv-theme-color-vinegar' },
                                { id: 'buttonocean', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/dark-3.css', styleId: 'Sv-theme-color-ocean' },
                                { id: 'buttonmountain', cssPath: '/appearance/themes/siyuan-theme-darkside/theme/dark-4.css', styleId: 'Sv-theme-color-mountain' }
                            ];
